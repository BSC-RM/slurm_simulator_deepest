...
#endif
					/* set frequency to best found for job */
					job_ptr->details->cpu_freq_max = job_ptr->details->cpu_freq_min = job_energy_info->best_freq;
			...
			while ((part_ptr = (struct part_record *)
				...
				if (j == used_modules) { 
					...
					if (use_energy_prediction) {
						...
						/* set frequency to best found for job */
						job_ptr->details->cpu_freq_max = job_ptr->details->cpu_freq_min = job_energy_info[j]->best_freq;
