...
// standard format in ascii (all columns from models)
	if (trace_format == 2) {
		...
		job_trace->dependency, &dummy, job_trace->module_list);
			...
			job_trace->wclimit = (int) ceil((float) job_trace->wclimit / 60.0f);
	...
	if (trace_format == 3) {
		...
		&job_trace->after_completion_job_id,
		...
		job_trace->wclimit = (int) ceil((float) job_trace->modular_requested_time / 60.0f);
