...
int main( int argc, char** argv )
{
	...
	char* partition = argv[4];  // Ana: New argument has been added and the changes have been made in writestatic 

        char* workload_name = argv[5];
	...
	//writestatic( jobspeclist, workload_name, "workload", partition );
	writestatic_swf( jobspeclist, workload_name, "swf", partition );
...
static void usage( char* prog_name )
{
	cerr << "correct usage: " << prog_name << " <nodes> <arrivalpattern> <jobs> <workload> [<k> <reductionfactor>]" << endl
		 ...
		 << "              <partition> contains the partition on which job is going to run" << endl
