...
int main(int argc, char* argv[])
{
	...
	i=0;
        while(p!=NULL){
            ...
            else if(i==4) { /*printf("Submit time: %s\n", p);*/ if (first_arrival == 0) first_arrival =atoi(p); job_arr[idx].submit = 11 + atoi(p) - first_arrival; }  // why submit cannot start from 0? Now it is changed from 10, since in real SLURM first submit is shifted 10-11s from the SLURM start. 
	    ...
	    else if(i==10) { /*printf("%s\n", p);*/ strcpy(job_arr[idx].partition, p); }   
            else if(i==11) { /*printf("%s\n", p);*/ strcpy(job_arr[idx].jobname, p); }   
            else if(i==12) { /*printf("%s\n", p);*/ if(strcmp(p,"nodep")) strcpy(job_arr[idx].dependency, p); }
            else if(i==13) { /*printf("Priority: %s\n", p);*/ job_arr[idx].priority = atoi(p); }
            //else if(i==14) { /*printf("Modules list: %s\n", p);*/ strcpy(job_arr[idx].module_list,p); }
            //else printf(" %s\n", p);   
            p = strtok(NULL," \t");
            i++; 
        }
        ...
        printf("%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%d;%s;%d;%d;%d;%s;%d;%d;%d\n",
                job_arr[idx].job_id,
                job_arr[idx].submit,
                -1,
                job_arr[idx].duration,
...
//jobid;arrival;-1;wclimit;size;-1;-1;-1;-1;-1;1;tester;-1;-1;1;partition;-1;-1;modulelist
//4999;2814;-1;50;16;-1;-1;-1;-1;-1;1;tester;-1;-1;1;dam;-1;-1;dam,esb
//5000;2815;-1;5;24;-1;-1;-1;-1;-1;1;tester;-1;-1;0;esb;-1;-1;-1 
        idx++;
    }
